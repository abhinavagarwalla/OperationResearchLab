...
int main()
{
	...
	//finding out value of v^- (min max wala)
	float colmax[10]={-1000};
	...
	for(int i=0;i < eqn;i++)
	{
		...
		{
			temp[i][j] = mat[i][j];
			if(colmax[j] < mat[i][j])colmax[j]=mat[i][j];
		}
	}
	
	float minma = 19900;
	for(int i=0;i<in_var;i++){
		if(minma > colmax[i])minma = colmax[i];
	}

	for(int i=0;i<in_var;i++)
	{
		temp[eqn][i] = -1.0/minma;
	}
	
	for(int i=0;i<eqn;i++){temp[i][in_var]=minma;}
