#include<bits/stdc++.h>
using namespace std;
void Matrix_inverse(double Br[][10], double Br_inv[][10], int n)
{
	int r=0,i,j,k, m=1;
	...
	while(r!=­1)
	{
		...
		for(i=0;i<n;i++)eta[i][0] = ­1*e[i][0]/e[r][0];
		eta[r][0] /= ­1*e[r][0];
		for(i=0;i<n;i++)
			for(j=0;j<n;j++)
			{
...
/*INPUTS START*/
	...
	for(i=0;i<m;i++)cin>>B[i][0];
	for(i=0;i<n;i++)vars[i][0]=0.0005;
	index=n;
	for(i=0;i<N;i++)
	{
	...
	cout<<"Iteration 0: ";
	for(i=0;i<N;i++)cout<<vars[i][0]<<" ";
	cout<<endl;
	/*INPUTS END*/
	while(1)
	{
		for(i=0;i<N;i++)D[i][i]=vars[i][0];
		double A_bar[10][10],A_barT[10][10], C_bar[10][10], P[10][10], Cp[10][10], X_bar[10][10],
		vars_1[10][10], interim_1[10][10], interim_2[10][10], interim_3[10][10], interim_4[10][10], vars_2[10][10];
		...
		Matrix_inverse(interim_1, interim_2, m);
		...
		for(i=0;i<N;i++)X_bar[i][0]=1+((alpha*Cp[i][0])/v);
		double diff[N];
		multiplication(D, X_bar, vars_1, N, N, N, 1);
		for (i=0;i<N;i++)diff[i] = abs(vars[i][0] ­ vars_1[i][0]);
